...
public class GameManager : MonoBehaviour {
	...
	public void targetHit (int scoreAmount, float timeAmount)
	{
		...
		string scoreValue = "", timeValue = "";

		if (scoreAmount > 0) scoreValue = "Score +" + scoreAmount.ToString();
		else if (scoreAmount < 0) scoreValue = "Score " + scoreAmount.ToString();

		if (timeAmount > 0) timeValue = "Time +" + timeAmount.ToString();
		else if (timeAmount < 0) timeValue = "Time " + timeAmount.ToString();
		...
		// update the text UI
		mainTimerDisplay.text = currentTime.ToString ("0.00");
		HitTextTimer.htt.SetText(scoreText);
	}
